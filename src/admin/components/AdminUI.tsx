  ...
  return (
    <div className="min-w-0">
      <header className="mb-5 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div className="min-w-0"><p className="eyebrow">{eyebrow}</p><h1 className="display-title mt-1 text-3xl sm:text-4xl">{title}</h1><p className="mt-2 max-w-3xl text-sm text-cls-ink/65">{description}</p></div>
...
export function StatCard({ label, value, detail, icon: Icon, tone = "paper" }: { label: string; value: string; detail: string; icon: LucideIcon; tone?: "paper" | "sage" | "honey" }) {
  ...
  return <article className={`${bg} min-w-0 rounded-2xl border border-cls-line p-4 shadow-paper`}><div className="flex items-start justify-between gap-3"><p className="text-xs font-bold text-cls-ink/60">{label}</p><Icon className="h-5 w-5 shrink-0 text-cls-primary" /></div><p className="mt-3 truncate text-2xl font-black text-cls-primary-dark">{value}</p><p className="mt-1 text-[11px] text-cls-ink/55">{detail}</p></article>;
}
...
export function Panel({ title, description, children, className = "" }: { title: string; description?: string; children: React.ReactNode; className?: string }) {
  return <section className={`rounded-2xl border border-cls-line bg-cls-paper shadow-paper ${className}`}><header className="border-b border-cls-line px-4 py-3 sm:px-5"><h2 className="font-sans text-sm font-black text-cls-primary-dark">{title}</h2>{description && <p className="mt-0.5 text-[11px] text-cls-ink/55">{description}</p>}</header><div className="min-w-0 p-4 sm:p-5">{children}</div></section>;
}

export function EmptyState({ title, text }: { title: string; text: string }) {
  return <div className="flex min-h-44 flex-col items-center justify-center px-4 text-center"><span className="flex h-11 w-11 items-center justify-center rounded-full bg-cls-sage text-cls-primary"><Inbox className="h-5 w-5" /></span><h3 className="mt-3 font-sans text-sm font-black">{title}</h3><p className="mt-1 max-w-sm text-xs text-cls-ink/55">{text}</p></div>;
}
